	...
	public function set_downloads( $downloads_array ) {
		...
		foreach ( $downloads_array as $download ) {
			...
			try {
				...
				// If the download had already been stored but only now has become disabled (this could be due to a
				// change in allowed filetypes, or to the list of Approved Download Directories) then move it to a list
				// of disabled downloads.
				if ( ! $download->get_enabled() || isset( $existing_downloads[ $download_id ] ) ) {
